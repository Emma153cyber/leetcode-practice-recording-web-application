...
const Problmes = require("./Problems");
...
app.post("/api/session", (req, res) => {
  ...
  if (!users.isValidUsername(username)) {
    res.status(400).json({ error: "illegalUsername" });
    return;
  }
  ...
  if (!existingUserData) {
    users.addUserData(username, Problmes.makeApplicationList());
  }
  res.cookie("sid", sid);
  res.json(users.getUserData(username).getProblmes());
});
...
app.get("/api/Problmes/:id", (req, res) => {
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : "";
  if (!sid || !username) {
    res.status(401).json({ error: "authMissing" });
  ...
  if (!applicationList.contains(id)) {
    res
      .status(404)
      .json({ error: `noSuchId`, message: `No application with id ${id}` });
    return;
  }
...
app.put("/api/Problmes/:id", (req, res) => {
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : "";
  if (!sid || !username) {
    res.status(401).json({ error: "authMissing" });
...
app.patch("/api/Problmes/:id", (req, res) => {
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : "";
  if (!sid || !username) {
    res.status(401).json({ error: "authMissing" });
...
app.delete("/api/Problmes/:id", (req, res) => {
  const sid = req.cookies.sid;
  const username = sid ? sessions.getSessionUser(sid) : "";
  if (!sid || !username) {
    res.status(401).json({ error: "authMissing" });
